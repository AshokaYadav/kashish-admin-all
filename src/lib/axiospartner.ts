import axios from "axios";
import { store } from "@/store/store";
import { logout } from "@/store/features/auth/authSlice";
import { toast } from "sonner";

const baseURL = process.env.NEXT_PUBLIC_PARTNER_API || "https://api.partner.kashishindiapvtltd.com/";

export const axiosInstance = axios.create({
  baseURL,
  headers: {
    "Content-Type": "application/json",
  },
  timeout: 1000 * 60 * 10,
});

// Add token to requests
axiosInstance.interceptors.request.use((config) => {
  const token = store.getState().auth.token;
  if (config.data instanceof FormData) {
    // Don't manually set 'Content-Type', let browser set it with boundary
    config.headers["Content-Type"] = "multipart/form-data";
  } else {
    config.headers["Content-Type"] = "application/json";
  }
  if (token) {
    config.headers.Authorization = `Bearer ${token}`;
  }
  return config;
});

// Handle 401 responses
axiosInstance.interceptors.response.use(
  (response) => {
    if (response.data.err) {
      toast('Failed', {
        description: response.data.err || 'Error in Request'
      })
    }
    return response;
  },
  (error) => {
    try {
      toast('Error In Request', {
        description: error?.response?.data?.err || error?.response?.data?.message || 'Request Failed'
      })
    }
    catch ({ message }: any) {
      console.error('error', message);
      toast('Error', { description: 'failed to resolve response' })
    }
    console.log("error", error);
    if (error.response?.status == 401) {
      toast('Auth Expired', {
        description: 'Login in to continue'
      })
      store.dispatch(logout());
      window.location.href = '/';
    }
    return Promise.reject(error);
  }
);








// import axios from "axios";
// import { store } from "@/store/store";
// import { logout } from "@/store/features/auth/authSlice";
// import { toast } from "sonner";

// // ========================
// // BASE URLs
// // ========================
// const BASE_MAIN = process.env.NEXT_PUBLIC_MAIN_API || "";
// const BASE_WALLET = process.env.NEXT_PUBLIC_WALLET_API || "";
// const BASE_RECHARGE = process.env.NEXT_PUBLIC_RECHARGE_API || "";


// // ========================
// // CREATE CLIENT FUNCTION
// // ========================
// function createAxios(baseURL: string) {
//   const instance = axios.create({
//     baseURL,
//     headers: { "Content-Type": "application/json" },
//     timeout: 1000 * 60 * 10,
//   });

//   // REQUEST INTERCEPTOR
//   instance.interceptors.request.use((config) => {
//     const token = store.getState().auth.token;

//     if (config.data instanceof FormData) {
//       config.headers["Content-Type"] = "multipart/form-data";
//     } else {
//       config.headers["Content-Type"] = "application/json";
//     }

//     if (token) config.headers.Authorization = `Bearer ${token}`;

//     return config;
//   });

//   // RESPONSE INTERCEPTOR
//   instance.interceptors.response.use(
//     (response) => {
//       if (response.data.err) {
//         toast("Failed", {
//           description: response.data.err,
//         });
//       }
//       return response;
//     },
//     (error) => {
//       toast("Error In Request", {
//         description:
//           error?.response?.data?.err ||
//           error?.response?.data?.message ||
//           "Request Failed",
//       });

//       if (error.response?.status === 401) {
//         toast("Auth Expired", { description: "Login to continue" });
//         store.dispatch(logout());
//         window.location.href = "/";
//       }

//       return Promise.reject(error);
//     }
//   );

//   return instance;
// }

// // ========================
// // EXPORT MULTIPLE AXIOS CLIENTS
// // ========================
// export const axiosMain = createAxios(BASE_MAIN);
// export const axiosWallet = createAxios(BASE_WALLET);
// export const axiosRecharge = createAxios(BASE_RECHARGE);
