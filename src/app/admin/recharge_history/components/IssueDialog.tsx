...
export default function IssueDialog({ open, setOpen, id, mobile,ref_id }: { open: boolean, setOpen: (val: boolean) => void, id: string, mobile: string,ref_id:string }) {
    const [complaint, setComplaint] = useState<IComplaintExtended>({ ...defaultValue, id: id, ref_id:ref_id, mobile: mobile || '', status: 'OPEN' });
    ...
    useEffect(() => {
        fetch(id);
        setComplaint({ ...complaint, id, mobile,ref_id});
    }, [id, mobile,ref_id]);
                ...
                <DialogHeader>
                    <DialogTitle>
                        <div>
                            ...
                            <h6 className='text-sm text-gray-500'>No.: {ref_id}</h6>
