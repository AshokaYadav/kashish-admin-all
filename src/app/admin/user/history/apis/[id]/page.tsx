...
const PDFDownloadSection = dynamic(() =>
    import('@react-pdf/renderer').then(lib => {
        ...
        return function PDFDownloadComponent ({ rechargeData }: {rechargeData: ITxnHistory[]}) {
            ...
            const RechargeHistoryPDF = ({ data, name }: {data: ITxnHistory[]; name: string}) => (
                <Document>
                    <Page size="A4" style={styles.page}>
                        <Text style={styles.title}>Api Transaction History Report</Text>
        
                        <Text style={styles.subtitle}>Api Details</Text>
                        <Text style={{ fontSize: 10, marginBottom: 4, fontWeight: 'bold' }}>Name: {name}</Text>
                        ...
                        <View style={styles.summary}>
                            <Text style={styles.subtitle}>Recharge Summary</Text>
        
                            <View style={styles.row}>
                                <Text style={styles.label}>Total Recharges:</Text>
                                <Text style={styles.value}>{data.length}</Text>
                            </View>
        
                            <View style={styles.row}>
                                <Text style={styles.label}>Total Amount:</Text>
                                <Text style={styles.value}>₹{data.reduce((sum: number, item: any) => sum + (item.amount || 0), 0).toFixed(2)}</Text>
                            </View>
        
                            <View style={styles.row}>
                                <Text style={styles.label}>Total Commission:</Text>
                                <Text style={styles.value}>₹{data.reduce((sum: number, item: any) => sum + (item.commission || 0), 0).toFixed(2)}</Text>
                            </View>
            ...
            return (
                <div className="flex gap-4 mb-6">
                    <PDFDownloadLink
                        document={<RechargeHistoryPDF data={rechargeData} name='KASHISH API'/>}
...
export default function TransactionsPage() {
    ...
    return (
        <div className="p-6 space-y-6 bg-white h-full flex flex-col">
            ...
            {
                (data) ? <PDFDownloadSection rechargeData={data?.data || []} /> : (<></>)
            }
        </div>
    );
}