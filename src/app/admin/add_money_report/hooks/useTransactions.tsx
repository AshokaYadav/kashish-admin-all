...
export const useTransactions = (isApisWalletTxns = false) => {
    ...
    const [paginatedParams, setPaginatedParams] = useState<IPagintatedTxnRequest>({ ...defaultPaginationParams, id: user?.userId || '' });

    useEffect(() => {
        setPaginatedParams({ ...paginatedParams, id: user?.userId || '' });
    }, [user?.userId]);
    ...
    const { data, isLoading: isLoadingTxns, refetch } = useGetTransactions({ isApisWallet: isApisWalletTxns, ...paginatedParams, id: user?.userId || '' , dateRange});
