...
const TransactionPDFDownload = dynamic(
  () =>
    import("@react-pdf/renderer").then((lib) => {
      ...
      return function PDFDownloadComponent({
        rechargeData,
        user,
        email,
        mobile,
      }: {
        rechargeData: ITxnHistory[];
        user: string;
        email: string;
        mobile: string;
      }) {
        const styles = StyleSheet.create({
          ...
          tableCell: { fontSize: 8, margin: 3, textAlign: "center" },
          footer: {
            position: "absolute",
            bottom: 20,
            left: 0,
        ...
        const PDFDoc = ({ data }: { data: ITxnHistory[] }) => (
          <Document>
            <Page size="A4" style={styles.page}>
              ...
              <View style={styles.table}>
                <View style={[styles.tableRow, styles.tableHeader]}>
                  {["Type", "Amount", "Open", "Close", "Comm", "Status", "Date"].map(
                    (h) => (
        ...
        return (
          <PDFDownloadLink
            document={<PDFDoc data={rechargeData} />}
